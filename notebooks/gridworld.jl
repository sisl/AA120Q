...
mutable struct GridWorld <: MDP{Int, Symbol}
  S::Vector{Int}
  A::Vector{Symbol}
  T::Array{Float64,3}
  R::Matrix{Float64}
...
n_actions(g::GridWorld) = length(g.A)
n_states(g::GridWorld) = length(g.S)
reward(g::GridWorld, s::Int, a::Symbol) = g.R[s, g.actionIndex[a]]
transition_pdf(g::GridWorld, s0::Int, a::Symbol, s1::Int) = g.T[s0, g.actionIndex[a], s1]
discount(g::GridWorld) = g.discount
next_states(g::GridWorld, s, a) = g.nextStates[(s, a)]
state_index(g::GridWorld, s) = s
action_index(g::GridWorld, a) = g.actionIndex[a]
...
function simulate(g::GridWorld, s::Int, a::Symbol)
    probs = Float64[]
